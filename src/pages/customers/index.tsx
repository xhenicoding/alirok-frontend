...
import { customerAppRoutes, dashboardAppRoutes } from 'helpers/appRoutes'
...
const Customers = () => {
  ...
  const [customerInviteListData, setCustomerInviteListData] = useState<any>({})
  ...
  const fetchAllCustomerList = async () => {
    try {
      ...
      const { data } = await rokApiV2().get(
        customerApiRoutes.INVITE_CONNECTED_CUSTOMER
      )
      ...
      console.log(
        'Error in fetching customer list data',
  ...
  const handleOnGroupBy = (
    ...
    for (const [key, value] of Object.entries(groupByCategory)) {
      const tmpData = value as Array<ICustomerInviteList>
    ...
    setCustomerInviteListData(finalData)
  }
  ...
  const handleGlobalSortBy = (sortByField: string) => {
    ...
    for (const [category, groupByData] of Object.entries({
      ...customerInviteListData
    })) {
      tmpAccountData[category] = {}

      const tmpGroupByData = groupByData as ICustomerInviteList[]
  ...
  const handleIndividualSortData = (options: ISortData): void => {
    ...
    const tmpData = { ...customerInviteListData }

    handleSorByColumnData(tmpData[category], sortColumn, sortOrder)
  }

  const renderLegalName = (row: ICustomerInviteList) => {
    if (row?.name) {
  ...
  return (
    <>
      {isFetching && <Loader />}
      <S.AddVendorIcon onClick={() => push(customerAppRoutes.INVITE)}>
        <Icon
          name="plus"
          width="50px"
          height="50px"
      ...
      <S.MainContainer>
        ...
        {!isFetching && state?.customer.connected.data?.length === 0 && (
          <NoDataFound content="You don't have any customers yet" />
        )}
        {!isFetching && state?.customer.connected.data?.length > 0 && (
          <>
        ...
        {Object.keys({ ...customerInviteListData }).map(
            ...
            <S.AccordionWrapper key={categoryKey}>
              ...
              >
                <List
                  ...
                  rows={[...customerInviteListData[category]].map(
                    (row: ICustomerInviteList) => ({
