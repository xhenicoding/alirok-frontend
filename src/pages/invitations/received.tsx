...
const InviteReceived = () => {
  ...
  const fetchAllInvitationsReceivedList = async () => {
    try {
      ...
      const { data } = await rokApiV2().get(invitationsApiRoutes.RECEIVED)
  ...
  const handleOnGroupBy = (
    ...
    const stateData = state.invitations.received.data
  ...
  const renderFilter = () => (
    <>
      ...
      <S.Filter>
        Group by:
        <FilterSelect
          options={[...INVITATIONS_GROUP_BY_COLUMNS]}
          selected={groupBy}
  ...
  return (
    <>
      ...
      <S.MainContainer>
        ...
        {!isFetching && state?.invitations?.received?.data?.length === 0 && (
          <NoDataFound content="You don't have any invitations yet" />
        )}
        {!isFetching && state?.invitations?.received?.data?.length > 0 && (
          <>
                        ...
                        },
                        {
                          ...
                          node: (
                            <S.StatusChip
                              status="accept_now"
                              onClick={() => {
                                let url = customerAppRoutes.ACCEPT_INVITE(
                                  row.company_relationship_uuid
                                )

                                if (row.category === 'vendors') {
                                  url = vendorAppRoutes.ACCEPT_INVITE(
                                    row.company_relationship_uuid
                                  )
                                }
...
InviteReceived.getLayout = (page: React.ReactNode) => (
