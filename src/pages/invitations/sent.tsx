...
const InviteSent = () => {
  ...
  const fetchAllInvitationsSentList = async () => {
    try {
      setIsFetching(true)
      ...
      const { data } = await rokApiV2().get(invitationsApiRoutes.SENT)
  ...
  const handleOnGroupBy = (
    ...
    const stateData = state.invitations.sent.data
  ...
  return (
    <>
      ...
      <S.MainContainer>
        ...
        {!isFetching && state?.invitations?.sent?.data?.length === 0 && (
                    ...
                    (row: IVendorInviteList) => ({
                      ...
                      elements: [
                        ...
                        },
                        {
                          ...
                          node: (
                            <S.StatusChip
                              status={(row.connected || '').toLowerCase()}
                            >
                              {row.connected}
                            </S.StatusChip>
                          )
...
InviteSent.getLayout = (page: React.ReactNode) => (
