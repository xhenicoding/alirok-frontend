...
export function SenderAddressInput({ onFinish, close }: IProps) {
  const { t } = useLocale()
  ...
  const {
    ...
    defaultValues: {
      ...state.parcelBooking.data.sender?.address,
      formattedAddress: ''
    }
  })
  ...
  return (
    <S.Container>
      ...
      {state.quote.form.whereFrom.step === 1 && (
        <>
          <S.FormWrapper>
            <S.Form key={2} onSubmit={handleAddressSubmit(onAddressSubmit)}>
              <S.Top>
                <S.BackContentDesk>
                  <S.BackIcon
                    onClick={() => {
